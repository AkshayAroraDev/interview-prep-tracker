...
A3["Supabase Auth Session Check<br/>lib/supabase/auth.ts::getCurrentAuthState()<br/>uses lib/supabase/browser.ts::supabase"]
...
A10["Save to Supabase<br/>supabase-state-repository.ts::insertInitialUserState()<br/>lib/supabase/browser.ts::supabase.from('user_state').insert(...)"]
...
%% =========================
subgraph SEC2["SECTION 2 - Google Authentication SSR Flow"]
direction TB
B0["User"]
...
B4["Google OAuth"]
B5["OAuth callback route<br/>app/auth/callback/route.ts::GET"]
B6["Server Supabase client<br/>lib/supabase/server.ts::createServerSupabaseClient()"]
B7["Exchange code for session<br/>supabase.auth.exchangeCodeForSession(code)"]
B8["Redirect to /"]
B9["Auth Listener<br/>lib/supabase/auth.ts::listenForAuthChanges()<br/>-> supabase.auth.onAuthStateChange(...)"]
B10["Session State<br/>AuthState { user, session, isLoading, error }"]
B11["AuthProvider updated<br/>setAuthState(state)"]
B12["Tracker Loaded<br/>hooks/use-interview-tracker.ts useEffect on user<br/>-> loadUserStateByUserId(user.id)"]
B13["UI Rendered"]
...
C8["Browser Supabase client<br/>lib/supabase/browser.ts::supabase<br/>.from('user_state').upsert(...)"]
...
C15["RLS policy gate<br/>auth.uid() must match user_id"]

C0 --> C1 --> C2 --> C3 --> C4 --> C5
C3 --> C6 --> C7 --> C8 --> C15 --> C9 --> C10
C10 -- "success" --> C11 --> C13
C10 -- "failure" --> C12 --> C13
C4 --> C14
end
...
%% =========================
%% SECTION 4: AI CHAT SSR
%% =========================
subgraph SEC4["SECTION 4 - AI Chat Route (SSR + Personalized Prompt)"]
direction TB
D0["User sends assistant message"]
D1["POST /api/chat<br/>app/api/chat/route.ts::POST"]
D2["Server Supabase client<br/>createServerSupabaseClient()"]
D3["Authorize request<br/>supabase.auth.getUser()"]
D4{"Authorized user?"}
D5["401 Unauthorized"]
D6["Load persisted state<br/>from user_state by user_id"]
D7["Build personalized prompt<br/>lib/ai/prompts.ts::buildPrompt(state, message)"]
D8["Generate model output<br/>lib/ai/gemini.ts::generateResponse(prompt)"]
D9["Return JSON { response }"]
...
B11 -. "auth user available" .-> A4
A6 -. "topic interaction" .-> C0
A6 -. "assistant message" .-> D0
...
style SEC4 fill:#f3f2ff,stroke:#5643d1,stroke-width:2px,color:#1f1854
...
classDef ai fill:#eef2ff,stroke:#4f46e5,stroke-width:1.5px,color:#1e1b4b

class A0,A1,A5,A6,A11,A12,A13,B0,B1,B2,B11,B12,B13,C0,C1,C2,C3,C4,C5,C13,C14,D0,D1,D9 react
class A2,A3,A4,B3,B4,B5,B6,B7,B8,B9,B10,D3,D4,D5 auth
class A7,A8,A9,A10,C6,C7,D6,D7 repo
class C8,D2 supa
class C9,C15 db
